...
check(
  'carries the rail marker the probe finds it by',
  withVersion.includes('data-taylorurl-bar'),
  withVersion.slice(0, 60)
)
check(
  'carries the right-hand group marker',
  withVersion.includes('data-tu-right'),
  'no [data-tu-right]'
)
check('links home', withVersion.includes('href="https://www.taylorurl.com"'), 'no home link')
check(
  'links to status',
  withVersion.includes('href="https://www.taylorurl.com/status"'),
  'no status link'
)
...
check(
  ...
  bare.includes(String(new Date().getFullYear())),
  'no year rendered'
)
